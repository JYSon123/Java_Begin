...
public class BubbleSortMain {
	...
	public static void main(String[] args) {
		...
		*/
		 ...
		 // 자리를 바꾸기 전에 임시저장소에 담아뒀다가, 옮기고 다시 담아준다
		 
		 
		 // === 오름차순 정렬하기 === //
		 for(int i=0; i<numArr.length-1; i++) {
			 // numArr.length-1은 비교할 대상을 추출할 개수  ==> 9  7  3  5
			 // 비교할 대상을 추출할 개수는 4개가 된다. 
   	         //for(int i=0; i<4; i++) {
			 for(int j=0; j<numArr.length-1-i; j++) {   // i=0; 일때는 조건식이  j<4; 되어야만 4번 비교한다.
 // i가 맨 처음에는 0 그리고 1씩 감소하기 때문에!i가 1 증가하면 j가 감해진다.  // i=1; 일때는 조건식이  j<3; 되어야만 3번 비교한다.
				                                        // i=2; 일때는 조건식이  j<2; 되어야만 2번 비교한다.
				                                        // i=3; 일때는 조건식이  j<1; 되어야만 1번 비교한다. 
			
				 if(numArr[j] >  numArr[j+1]) { //만약 그 다음방에 있는 값이 더 작으면
				 // numArr[0] >  numArr[1]
				 //    9      >      7
	 
				
				 /*
				  	  int temp = numArr[0];  // temp <= 9
		 ...
		 int[] numArr2 = {9,7,3,5,1};  //내림차순으로 한번 해보자!
