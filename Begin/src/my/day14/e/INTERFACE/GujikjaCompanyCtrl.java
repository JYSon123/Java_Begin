...
public class GujikjaCompanyCtrl  implements InterGujikjaCompanyCtrl {
	...
	@Override
	public boolean registerGujikja(Scanner sc, Member[] mbrArr) { // 공유를 하기 때문에 기존의 register에서, 구분을 위해 registerGujikja로 바꿔준다.
	...
	public void search(int ageline, Member[] mbrArr) {
		...
		for(int i=0; i<Member.count; i++) {
			if( mbrArr[i] instanceof Gujikja &&  // 이게 참이라면, 구직자로 형변환을 해서 getAge를 해준다.
					((Gujikja)mbrArr[i]).getAge()/10*10 == ageline) { // 20        24        29        ==> 20
				                                      // 20/10*10  24/10*10  29/10*10  ==> 20
				isExists = true;
				System.out.println(mbrArr[i].showInfo());  // 부모클래스의 showInfo()는 추상메소드화 되어있다. 그래서 자식 메소드에서 showInfo()를 재정의를 꼭 해야하며, 자식메소드의 showInfo()가 출력이 된다.
			}
		}// end of for-------------------------
	 ...
	 public void search(String gender, Member[] mbrArr) {
		...
		for(int i=0; i<Member.count; i++) {
			...
			if(mbrArr[i] instanceof Gujikja) {
				...
				String n_gender = ((Gujikja)mbrArr[i]).getJubun().substring(6); // Member에는 getJubun()이 없어서 오류가 난다. 그래서 형변환을 해줘야한다.
				//     "1"   "2"   "3"   "4"
				...
				if("남".equals(gender)) {
					if("2".equals(n_gender) || "4".equals(n_gender)) {
						...
						System.out.println(mbrArr[i].showInfo()); //showInfo()는 이미 자식 클래스에서 재정의를 해둬서 형변환이 필요없다.
					}
					}
					else {
	...
	public void search(int ageline, String gender, Member[] mbrArr) {
		...
		for(int i=0; i<Gujikja.count; i++) {
			...
			if(mbrArr[i] instanceof Gujikja) {
				...
				if(ageline != ((Gujikja)mbrArr[i]).getAge()/10*10) {
					continue;
				}
				else {
	...
	public Gujikja login(Scanner sc, Member[] mbrArr) {
		...
		for(int i=0; i<Member.count; i++) {
			String id =	mbrArr[i].getId();          // 이 값은 절대 null이 아니다. 가입 시점에서 null은 이미 걸러졌기 때문이다.
			String passwd = mbrArr[i].getPasswd();  // 이 값은 절대 null이 아니다. 가입 시점에서 null은 이미 걸러졌기 때문이다.
		
		
			if(id.equals(input_id) && passwd.equals(input_passwd)) {
				// id 및 암호가 일치했다라면
				
				if(mbrArr[i] instanceof Gujikja) { // 실제로 구직자 클래스로 되어진 인스턴스입니까? 라는 뜻이다.
					loginGu = (Gujikja)mbrArr[i];  // 실제 구직자에 속한게 맞나 봐야 하기 때문에, 구직자로 형변환을 해준다.
					break;
				}
	...
	// === 구인회사 로그인 ===
	@Override
	public Company login(Member[] mbrArr ,Scanner sc) { //오버로딩 꼼수!
	...
	// === 구직자 또는 구인회사 모두 보기 ===
	@Override
	public void showAll(Member[] mbrArr, int no) { // int no는 Main의 구분자가 1이냐, 2냐를 파악해준다.
		...
		if(no==1) {
			...
			for(int i=0; i<Member.count; i++) {
				...
				if(mbrArr[i] instanceof Gujikja) { // 구직자입니까?
					System.out.println(mbrArr[i].showInfo());
				}
			...
			// 모든 구인회사 보여주기 자르지않고 아래의 showAll을 호출을 하는 방ㅂ버을 썻다.
			showAll(mbrArr);
	...
	//== 구인회사 모두보기
	@Override
	public void showAll(Member[] mbrArr) { // 만약 회사 정보가 없을 경우도 고려를 해야한다.
