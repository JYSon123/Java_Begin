...
public class GujikjaCompanyMain {
	...
	public static void main(String[] args) {
		...
		InterGujikjaCompanyCtrl ctrl = new GujikjaCompanyCtrl();  // 첫번째 팀원이 만든것 GujikjaCompanyCtrl 클래스를
	//	InterGujikjaCompanyCtrl ctrl = new GujikjaCompanyCtrl2();  // 두번째 팀원이 만든것 GujikjaCompanyCtrl 클래스를
			...
			switch (smenuNo) {
				...
				case "5": // 모든 구직자 보기 , 구직자가 아니라 구인회사로 들어왔을때만 보여지게 하자.
					...
					if(loginComp != null && loginGu == null) {
						ctrl.showAll(mbrArr,2 ); // 6번과 똑같기 때문에, 메소드를 다르게 해줘야한다! 
				...
				case "6": // 모든 구인회사 보기
					...
					if(loginComp == null && loginGu != null) {
						ctrl.showAll(mbrArr, 2);  //showAll과 겹치기 때문에, 오버로딩을 해주면 되지만 파라미터가 1개뿐이라 할수가 없다. 메소드 이름을 바꿔주거나 , 구분자를 하나 주거나 한다. 
	...
	static void searchMenu(Scanner sc, InterGujikjaCompanyCtrl ctrl, Member[] mbrArr) {
